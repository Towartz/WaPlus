...
function extractBody(message, msgType) {
  ...
  // NOTE: viewOnce is intentionally NOT unwrapped here — messageParser.js
  // already did it before calling extractBody. The message we receive is
  // already the inner payload (e.g. imageMessage, videoMessage, audioMessage).
  ...
  switch (msgType) {
    ...
    // ── ViewOnce (legacy — for old DB rows not yet re-parsed) ───────────────
    // For live messages viewOnce is unwrapped by messageParser before we're called,
    // so msgType will be the inner type (imageMessage/videoMessage/audioMessage).
    // These cases handle old DB rows stored before the parser fix.
    ...
    case "viewOnceMessageV2": {
      ...
      if (innerType === "audioMessage") return ""
      return ""
    }
