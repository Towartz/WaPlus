...
//   STATE_FILE          → userData/mods_state.json (enabled/disabled state)
//
// Platform userData:
//   Linux   → ~/.config/AuroraChat/
//   Windows → %APPDATA%\AuroraChat\
//   Mac     → ~/Library/Application Support/AuroraChat/
const { app: _app } = (() => { try { return require('electron') } catch (_) { return { app: null } } })()
const _USER_DATA_DIR = (_app && _app.getPath)
  ? _app.getPath('userData')
  : path.resolve(__dirname)  // dev fallback

const BUILTIN_PLUGINS_DIR = path.resolve(__dirname, "./plugins")  // read-only .asar
const USER_PLUGINS_DIR    = path.join(_USER_DATA_DIR, "plugins")  // writable userData
const STATE_FILE          = path.join(_USER_DATA_DIR, "mods_state.json")

// ─── Ensure user plugins dir exists (builtin dir di .asar, skip mkdir) ───────
if (!fs.existsSync(USER_PLUGINS_DIR)) fs.mkdirSync(USER_PLUGINS_DIR, { recursive: true })
...
class ModManager extends EventEmitter {
  ...
  // Scan 2 dir: builtin (.asar, read-only) + user (userData, writable)
  // User plugin dengan id sama akan override builtin.
  async _scanPlugins() {
    this.plugins.clear()

    // 1. Load built-in plugins dari .asar
    if (fs.existsSync(BUILTIN_PLUGINS_DIR)) {
      const entries = fs.readdirSync(BUILTIN_PLUGINS_DIR, { withFileTypes: true })
      for (const entry of entries) {
        if (!entry.isDirectory()) continue
        await this._loadPlugin(entry.name, BUILTIN_PLUGINS_DIR).catch(err => {
          this._logE(`Failed to load builtin plugin "${entry.name}":`, err.message)
        })
      }
    }

    // 2. Load user plugins dari userData (override builtin jika id sama)
    if (fs.existsSync(USER_PLUGINS_DIR)) {
      const entries = fs.readdirSync(USER_PLUGINS_DIR, { withFileTypes: true })
      for (const entry of entries) {
        if (!entry.isDirectory()) continue
        await this._loadPlugin(entry.name, USER_PLUGINS_DIR).catch(err => {
          this._logE(`Failed to load user plugin "${entry.name}":`, err.message)
        })
      }
    }
  }

  async _loadPlugin(folderName, baseDir = BUILTIN_PLUGINS_DIR) {
    ...
    this.plugins.set(manifest.id, {
      ...
      error:   loadError,
      folderName,
      baseDir,  // [FIX-ASAR] track apakah builtin atau user plugin
    })
  ...
  _createContext(pluginId) {
    ...
    return {
      ...
      // ── DB ────────────────────────────────────────────────────
      getDB: () => {
        try { return require("../baileys/dbHandler") } catch { return null }
      },
  ...
  // ─── PLUGIN STORAGE ───────────────────────────────────────
  // Storage (data.json) selalu di USER_PLUGINS_DIR — writable untuk semua plugin termasuk builtin
  _pluginStoragePath(pluginId) {
    const storageDir = path.join(USER_PLUGINS_DIR, pluginId)
  ...
  getPluginDetail(pluginId) {
    ...
    let source = ""
    try {
      source = fs.readFileSync(path.join(plugin.baseDir, plugin.folderName, "index.js"), "utf8")
    } catch (_) {}

    return {
      ...this.listPlugins().find(p => p.id === pluginId),
      source,
      isBuiltin: plugin.baseDir === BUILTIN_PLUGINS_DIR,
  ...
  openPluginFolder(pluginId) {
    ...
    const pluginDir = plugin
      ? path.join(plugin.baseDir, plugin.folderName)
      : USER_PLUGINS_DIR
    if (fs.existsSync(pluginDir)) shell.openPath(pluginDir)
  }

  // Create new plugin from template — always goes to USER_PLUGINS_DIR
  async createPlugin(id, name, description, hooks = null, author = "Anonymous", version = "1.0.0") {
    const slug    = id.replace(/[^a-z0-9_-]/gi, "-").toLowerCase()
    const dir     = path.join(USER_PLUGINS_DIR, slug)
    ...
    const manifest = { id: slug, name, version: version || "1.0.0", description, author: author || "Anonymous", hooks: selectedHooks }
...
`
    ...
    // Load the new plugin from USER_PLUGINS_DIR
    await this._loadPlugin(slug, USER_PLUGINS_DIR)
    this._notifyUI()
  ...
  async updatePluginHooks(pluginId, enabledHooks) {
    ...
    // Block editing builtin plugins (read-only .asar)
    if (plugin.baseDir === BUILTIN_PLUGINS_DIR) {
    ...
    await this._loadPlugin(plugin.folderName, plugin.baseDir)
    this._notifyUI()
    return { ok: true }
  }
  ...
  async deletePlugin(id) {
    ...
    // Block deleting builtin plugins
    if (plugin.baseDir === BUILTIN_PLUGINS_DIR) {
      return { ok: false, error: "Built-in plugins cannot be deleted" }
    }
    ...
    // Delete folder from disk (user plugins only)
    const dir = path.join(USER_PLUGINS_DIR, id)
