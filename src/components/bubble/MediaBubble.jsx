...
import { useMediaSrc, useIsDownloading, toBool, MediaLoadingSpinner, DownloadingPulse, MediaErrorPlaceholder, RichText, ExpandableText, pathToFileUrl } from "./utils"
...
export function ImageBubble({ msg, onMediaClick }) {
  ...
  // Track actual image decode state — NOT initialized to false blindly.
  // We use a ref+state pattern: imgRef lets us check naturalWidth instantly
  // (browsers cache decoded images — if already decoded, naturalWidth > 0 immediately).
  const imgRef = useRef(null)
  const [loaded, setLoaded] = useState(false)
  const [imgErr, setImgErr] = useState(false)
  // [FIX-HOOKS] Must be declared here (before any early returns) to satisfy React's
  // hooks invariant. Previously this was after the !src early return, causing
  // "Rendered fewer hooks than expected" crash when src was null.
  const [imgDims, setImgDims] = useState(null) // { w, h } | null
  ...
  // When src changes (new download arrived), reset error state.
  // Do NOT reset loaded here — if the img element already has the new src decoded
  // (browser cache hit), onLoad fires synchronously before this effect runs.
  const prevSrc = useRef(src)
  if (prevSrc.current !== src) {
    prevSrc.current = src
    if (imgErr) setImgErr(false)
    // Check immediately: if the img DOM node already decoded this src (cache hit),
    // mark as loaded right away so we never show the blurred placeholder.
    if (imgRef.current && imgRef.current.complete && imgRef.current.naturalWidth > 0) {
      if (!loaded) setLoaded(true)
    } else {
      // New src that isn't cached yet — show placeholder until onLoad fires
      setLoaded(false)
    }
  }

  // Callback ref: fires when the <img> DOM node is attached or updated.
  // If the browser already has this image decoded (naturalWidth > 0 on mount),
  // skip the blur entirely — this is the main fix for the "still blurry after
  // download" regression caused by loaded=false on every remount.
  ...
  if (needsManualDownload) {
    return (
      <div style={{ borderRadius: 8, overflow: "hidden" }}>
        ...
        {msg.body && <div className="media-caption" style={{ padding: "6px 10px 8px", fontSize: 13 }}><ExpandableText text={msg.body} /></div>}
      </div>
    )
  }

  // No src yet — file not downloaded or missing
  if (!src || imgErr) {
    const showRetryBtn = imgErr
    return (
  ...
  // src is available — render the real image.
  // The blurred thumbnail is only shown while loaded=false AND the image hasn't
  // decoded yet. On cache hits (revisiting a chat), loaded is set synchronously
  // via the callback ref so thumbnail never flashes.

  // Compute orientation-aware constraints:
  //   Portrait  (ratio < 0.65): narrow tall — 220px wide, up to 380px tall, contain
  //   Square-ish (0.65–1.5):    280×280, cover
  //   Landscape (ratio > 1.5):  full width, max 200px tall, cover
  const { bubbleW, bubbleMaxH, fitMode } = (() => {
    if (!imgDims) return { bubbleW: "100%", bubbleMaxH: 280, fitMode: "cover" }
    const ratio = imgDims.w / imgDims.h
    if (ratio < 0.65) return { bubbleW: 220, bubbleMaxH: 380, fitMode: "contain" }
    if (ratio < 1.5)  return { bubbleW: 280, bubbleMaxH: 280, fitMode: "cover" }
    return { bubbleW: "100%", bubbleMaxH: 200, fitMode: "cover" }
  })()

  return (
    <>
      <div style={{ lineHeight: 0, borderRadius: msg.body ? "8px 8px 0 0" : 8, overflow: "hidden", position: "relative", minHeight: loaded ? undefined : 120, width: bubbleW, maxWidth: "100%" }}>
        {/* Blurred placeholder — only visible while real image is decoding */}
        {!loaded && thumbnailSrc && (
          <img src={thumbnailSrc} alt="" draggable={false} aria-hidden
            style={{ position: "absolute", inset: 0, width: "100%", height: "100%",
              objectFit: "cover", filter: "blur(8px)", transform: "scale(1.05)",
        ...
        <img
          ...
          draggable={false}
          style={{
            display: "block", maxWidth: "100%", maxHeight: bubbleMaxH, width: "100%",
            objectFit: fitMode, cursor: "zoom-in",
            opacity: loaded ? 1 : 0,
            transition: loaded ? "none" : "opacity 0.2s ease",
...
export function VideoBubble({ msg, onMediaClick }) {
  ...
  // [FIX-HOOKS] Must be before any early return to satisfy React hooks invariant
  const [videoDims, setVideoDims] = useState(null) // { w, h } | null
  ...
  if (isGif) {
    return (
      <div className="media-img" style={{ position: "relative" }}>
        ...
        {msg.body && <div className="media-caption"><ExpandableText text={msg.body} /></div>}
      </div>
    )
  }

  // [FIX-ORIENTATION] Video dimensions set via onLoadedMetadata below
  const { vidW, vidMaxH } = (() => {
    if (!videoDims) return { vidW: "100%", vidMaxH: 260 }
    const ratio = videoDims.w / videoDims.h
    if (ratio < 0.65) return { vidW: 220, vidMaxH: 360 }
    if (ratio < 1.5)  return { vidW: 280, vidMaxH: 280 }
    return { vidW: "100%", vidMaxH: 200 }
  })()

  // Video thumbnail: use thumbnailSrc (data: URI) as instant poster when available.
  const hasThumbnailSrc = !!thumbnailSrc
  return (
    <div className="media-video" onClick={handleClick} style={{ cursor: "pointer", position: "relative", borderRadius: 8, overflow: "hidden", width: vidW, maxWidth: "100%" }} title="Klik untuk putar">
      {!thumbLoaded && hasThumbnailSrc && (
        <img src={thumbnailSrc} alt="" draggable={false} aria-hidden
          style={{ display: "block", width: "100%", maxHeight: vidMaxH, objectFit: "cover",
      ...
      <video
        ...
        onError={() => setErr(true)}
        style={{
          maxWidth: "100%", maxHeight: vidMaxH, display: "block", borderRadius: 8, width: "100%",
          opacity: (hasThumbnailSrc && !thumbLoaded) ? 0 : 1,
          position: (hasThumbnailSrc && !thumbLoaded) ? "absolute" : "relative",
        }}
      />
      ...
      {msg.body && <div className="media-caption" onClick={e => e.stopPropagation()}><ExpandableText text={msg.body} /></div>}
    </div>
  )
}
...
export function AlbumBubble({ msgs, onMediaClick, openMedia }) {
  ...
  const anyDownloaded = msgs.some(m => m.media_saved_path || m.mediaSavedPath || m.media_url)
  ...
  const handleClick = (msg, idx) => {
    const items = msgs.map(m => ({
      src: albumPathToSrc(m.media_saved_path || m.mediaSavedPath) || m.media_url || null,
  ...
  return (
    <div className="album-grid" style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 2, borderRadius: 8, overflow: "hidden", position: "relative" }}>
      {visible.map((msg, idx) => {
        const src = albumPathToSrc(msg.media_saved_path || msg.mediaSavedPath) || msg.media_url
